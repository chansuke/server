...
class btr_est_cur_t
{
  ...
  */
  ...
  /* Page id of the page to get on level down */
  page_id_t m_page_id;
  /* Current block */
  buf_block_t *m_block;
...
public:
  ...
  @param[out] tree_block reference to the element in the array of already
              visited blocks, the function adds gotten block to the array.
  @param[out] tree_savepoint reference to the element in the array of mtr
  ...
  @param[in]  zip_size page zip size.
  @param[in]  rw_latch latch mode for the page,
  @param[in]  level the page level
  @param[in,out] mtr mtr
  @return DB_SUCCESS on success or err code on error.
  */
  dberr_t get_block(buf_block_t* &tree_block, ulint &tree_savepoint,
                    ulint zip_size, ulint rw_latch,
                    ulint level, mtr_t &mtr)
  {
    dberr_t err= DB_SUCCESS;
    tree_savepoint= mtr_set_savepoint(&mtr);
    m_block=
        buf_page_get_gen(m_page_id, zip_size, rw_latch, nullptr, BUF_GET, &mtr,
    ...
    if (err != DB_SUCCESS)
    {
      ...
      if (err == DB_DECRYPTION_FAILED)
      {
        ib_push_warning((void *) nullptr, DB_DECRYPTION_FAILED,
                        "Table %s is encrypted but encryption service or"
                        " used key_id is not available. "
                        " Can't continue reading table.",
                        index()->table->name.m_name);
    ...
    ut_a(m_block);
    page_t *page= buf_block_get_frame(m_block);
  ...
  void search_on_page(ulint level, ulint root_height, bool left)
  {
  ...
  @param[in,out] offsets offsets array.
  @param[in,out] heap heap for offsets array */
  ...
  bool should_count_the_left_border()
  {
    ...
    if (dtuple_get_n_fields(&m_tuple) > 0)
    {
      ...
      result= !page_rec_is_supremum(page_cur_get_rec(&m_page_cur));
    }
    else
    {
      ut_ad(page_rec_is_infimum(page_cur_get_rec(&m_page_cur)));
  ...
  bool should_count_the_right_border()
  {
    ...
    if (dtuple_get_n_fields(&m_tuple) > 0)
    {
      ...
      result= (m_mode == PAGE_CUR_LE /* if the range is '<=' */
  ...
  /** Copies block pointer from another btr_est_cur_t(in the case if both
  path slots point to the same block.
...
static ha_rows
btr_estimate_n_rows_in_range_on_level(ulint level, btr_est_cur_t &left_cur,
                                      uint32_t right_page_no, ulint rw_latch,
                                      ha_rows n_rows_on_prev_level,
                                      bool &is_n_rows_exact, mtr_t &mtr)
{
  ha_rows n_rows= 0;
  uint n_pages_read= 0;
  /* Do not read more than this number of pages in order not to hurt
  performance with this code which is just an estimation. If we read
  this many pages before reaching right_page_no, then we estimate the
  average from the pages scanned so far. */
#define N_PAGES_READ_LIMIT 9
  ulint savepoint= 0;
  ...
  /* Add records from the left page which are to the right of
  ...
  do
  {
    ...
    /* Fetch the page. */
    block= buf_page_get_gen(page_id, zip_size, rw_latch, NULL, BUF_GET, &mtr,
                            &err, level == 0 && !index->is_clust());
    if (prev_block)
    ...
    /* TODO: ask Marko what does it mean */
    ...
    if (n_pages_read == N_PAGES_READ_LIMIT || page_id.page_no() == FIL_NULL)
    {
      ...
      through right_page_no, the tree must have changed
      in the meantime */
      goto inexact;
    }
...
left page, then if left and right pages are different, read a few pages to
the right, if the right page is reached, fetch it and count the exact number
of rows, otherwise count the estimated(see
...
static ulint btr_estimate_n_rows_in_range_low(dict_index_t *index,
                                              ...
                                              btr_pos_t *range_end)
{
  ...
  DBUG_ENTER("btr_estimate_n_rows_in_range_low_2");
  ...
  ut_ad(dict_index_check_search_tuple(index, range_start->tuple));
  ut_ad(dict_index_check_search_tuple(index, range_end->tuple));
  ut_ad(!dict_index_is_ibuf(index) || ibuf_inside(&mtr));
  ut_ad(dtuple_check_typed(range_start->tuple));
  ut_ad(dtuple_check_typed(range_end->tuple));
  ut_ad(!(index->type & DICT_FTS));
  ut_ad(index->page != FIL_NULL);
  ut_ad(!dict_index_is_spatial(index));
...
search_loop:
  rw_latch=
      height ? (srv_read_only_mode ? RW_NO_LATCH : RW_S_LATCH) : RW_S_LATCH;
  ut_ad(n_blocks < BTR_MAX_LEVELS * 2);
  err= p1.get_block(tree_blocks[n_blocks], tree_savepoints[n_blocks], zip_size,
                    rw_latch, height, mtr);
  if (err)
    goto func_exit;
  ++n_blocks;
  ...
  if (height == 0)
  {
    ...
    /* Don't unlatch p2 parent as p2 has not yet been latched, that is
    why blocks_added is necessary */
  ...
  else
  {
    ...
    if (diverged_lot)
      n_rows= btr_estimate_n_rows_in_range_on_level(
          height, p1, p2.page_id().page_no(), rw_latch, n_rows,
          is_n_rows_exact, mtr);
    err= p2.get_block(tree_blocks[n_blocks], tree_savepoints[n_blocks],
                      zip_size, rw_latch, height, mtr);
    if (err != DB_SUCCESS)
      goto func_exit;
    ++n_blocks;
    blocks_added= 2;
  ...
  if (height == root_height)
    ut_a(btr_page_get_level(buf_block_get_frame(p2.block())) == height);

  p2.search_on_page(height, root_height, false);
  ...
  if (!diverged && (p1.nth_rec() != p2.nth_rec()))
  {
    ...
    if (p1.nth_rec() < p2.nth_rec())
    {
      ...
      if (n_rows > 0)
      {
        ...
        the two borders pointed to by p1
        and p2, so on the level below the
        slots will point to non-adjacent
        pages. */
        diverged_lot= true;
        divergence_level= root_height - height;
      }
    }
    else
    {
      /* It is possible that
      p1->nth_rec >= p2->nth_rec
      if, for example, we have a single page
      tree which contains (inf, 5, 6, supr)
      and we select where x > 20 and x < 30;
      in this case p1->nth_rec will point
      to the supr record and p2->nth_rec
      will point to 6. */
      n_rows= 0;
...
func_exit:
  ...
  if (is_n_rows_exact)
  {
    ...
    the leaf level. */
    if (p1.page_id().page_no() == p2.page_id().page_no() &&
        p1.nth_rec() == p2.nth_rec())
    {
  ...
  if (root_height > divergence_level && !is_n_rows_exact)
